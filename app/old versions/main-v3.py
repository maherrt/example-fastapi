...
def create_user(user:schemas.UserCreate ,db: Session = Depends(get_db) ):
    #hash the password
    hashed_paword= utils.hash(user.password)
    user.password= hashed_paword
...
@app.get("/users/{id}",response_model=schemas.UserOut) #,status_code=status.HTTP_201_CREATED,response_model=schemas.UserOut)
